        ...
        <div class="fj-search-inline">
          ...
          <el-form inline label-position="left">
            ...
            <el-col :lg="6" :xl="6">
              ...
              <el-form-item>
                <el-input
                  ...
                  class="search-input"
                >
                  <el-button slot="append" @click="getRecruits">搜索</el-button>
                </el-input>
              </el-form-item>
            </el-col>
            <el-col :lg="8" :xl="8" class="time-item">
              ...
              <el-form-item>
                <form style="display:none;" name="exportForm" :action="ajaxUrlDNN + '/exportRecruits?nowUser=' + nowUser + '&endTime=' + searchForm.endTime + '&deptId=' + searchForm.deptId + '&startTime=' + searchForm.startTime + '&page=' + currentPage + '&nameOrPhone=' + searchForm.nameOrPhone + '&rows=' + pageSize"
                      method="post" enctype="multipart/form-data"></form>
                <el-button type="primary" @click="openMFSpopMultiple"><span>批量审核</span></el-button>
                <el-button plain @click="exportRecruits"><!-- <i class="el-icon-upload2"></i> --><span>导出</span></el-button>
                <el-button plain @click="addRecruit"><span>添加</span></el-button>
              </el-form-item>
            </el-col>
          </el-form>
        </el-row>
        </div>
        <el-table :data="recruits" class="el-tables" @selection-change="handleSelectionChange">
          ...
          <el-table-column label="审核结果" show-overflow-tooltip prop="status" width="80px">
            <template slot-scope="scope">
              <span class="circle-status" :class="((scope.row.status == 0) || (scope.row.status == 1 && scope.row.step != 5)) ? 'grey' : (scope.row.status == 1 && scope.row.step == 5) ? 'green' : 'red'">
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <a href="javascript:;" class="ope-txt" @click.stop="gotoDetail(scope.row)">详情</a>
              <a v-if="(scope.row.status == 2) || (scope.row.status == 1 && scope.row.step == 5)" href="javascript:;" class="ope-txt" @click.stop="deleteRecruit(scope.row)">删除</a>
              <a v-else href="javascript:;" class="ope-txt" @click.stop="updateRecruit(scope.row)">审核</a>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    exportRecruits: function(){ // 导出
      document.forms['exportForm'].submit();
    },
    openMFSpopMultiple: function(){ // 批量审核
      if(!this.multipleSelection || !this.multipleSelection.length){
        this.$message({type:'warning', message:'请选择要进行批量审核的人员招聘信息！'});
        return;
      }
      var tmpObj = _.find(this.multipleSelection, function(item){
        return item.status == 2 || (item.status == 1 && item.step == 5);
      },this);
      if(tmpObj){
        this.$message({type:'warning', message:'所选的人员招聘信息包含已审核完成的，请重新选择！'});
        return;
      }
      var step = this.multipleSelection[0].step;
      var tmpObj1 = _.find(this.multipleSelection, function(item){
        return item.step != step;
      },this);
      if(tmpObj1){
        this.$message({type:'warning', message:'所选的人员招聘信息流程不一致，请重新选择！'});
        return;
      }
      var ids = [];
    ...
    changeSearchTime: function() {
      if (this.searchForm.searchTime) {
        this.searchForm["startTime"] = fjPublic.dateFormatYYMMDD(this.searchForm.searchTime[0]);
        this.searchForm["endTime"] = fjPublic.dateFormatYYMMDD(this.searchForm.searchTime[1]);
      } else {
    ...
    getRecruits: function() {
      ...
      $.ajax({
        url: fjPublic.ajaxUrlDNN + "/getRecruits",
        type: "POST",
        data: vm.searchForm,
        dataType: "json",
        success: function(data) {
          vm.recruits = null;
          vm.recruits = data.list;
          vm.total = data.total;
          _.each(vm.recruits, function(item, i) {
    ...
    gotoDetail: function(data){  // 招聘详情
      ...
      this.$router.push({path:'recruit-detail', query:{recruitId: data.id, recruitStep: data.step}});
    },
    addRecruit: function () {
      var defer = $.Deferred();
      var vm = this;
