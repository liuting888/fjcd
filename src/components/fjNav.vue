...
<script>
  ...
  window.fjSideNav = {
    ...
    setFirstList: function () {
      _.each(this.navData, function (item, i) {
      ...
      this.secondMenus = this.box.find('.fj-sider-sMenu:not(.third)');
    ...
    setSecondList: function (data) {
      ...
      _.each(data, function (item) {
        if(item.children){
          htmlArr.push(_.template(
            '<li class="fj-sider-sMenu_item has-children" data-title="{{data.title}}" data-route="{{data.route}}"><span>{{data.title}}</span>'+this.setThirdList(item.children)+'</li>', {
              variable: 'data'
            })(item));
        }else{
          htmlArr.push(_.template(
    ...
    setThirdList:function(data){ //设置三级菜单
      if(!data.length)return '';
      var htmlArr = [];
      _.each(data, function (item) {
        htmlArr.push(_.template(
          '<li class="fj-sider-sMenu_item third" data-title="{{data.title}}" data-route="{{data.route}}">{{data.title}}</li>', {
            variable: 'data'
          })(item));
      },this);
      return '<ul class="fj-sider-sMenu third">' + htmlArr.join('') + '</ul>';
    },
    firstItemsClickEvent:function (ev) {
      var $this = $(ev.currentTarget),
        ...
        pos = $this.offset();
      //
      if($this.hasClass('is-opened')&&$this.hasClass('is-own')&&this.boxParent.hasClass('sider-toggle')){
      ...
      if ($this.hasClass('is-opened')){
        ...
        this.secondMenus.eq(index).stop(true, true).slideUp();
      } else {
        ...
        if(this.secondMenus.eq(index).hasClass('is-empty')){
          ...
          this.secondMenus.eq(index).stop(true, true).slideDown();
        }
        this.slider.hide().css({
          'transform': 'translateY(' + (pos.top-this.parentPt) + 'px)'
        });
        this.secondMenus.find('.fj-sider-sMenu_item').removeClass('is-select');
      }
      //点击没有二级菜单的一级菜单项
    ...
    noSecondMenuClickEvent:function($this,index){ //点击没有二级菜单的一级菜单项
      /* 取消箭头的旋转 */
      this.cancelArrowsRotate();
    ...
    setEvents: function () { //设置事件
      this.boxParent.on('mouseleave',_.bind(function(){
        this.slider.hide();
      },this));
      //展开收起菜单
      this.firstItems.on({
        ...
        'mouseenter': _.bind(function (ev) {
          ...
          if (this.boxParent.hasClass('sider-toggle')){
            ...
            if(!this.secondMenus.eq(index).hasClass('is-empty')){
              ...
              this.arrows.eq(index).attr('src', this.arrows.eq(index).attr('data-open')).css('transform', 'rotate(0deg)');
            }
            return false;
          }
          var pos = $this.offset();
      ...
      //点击切换组件
      this.box.on('click', '.fj-sider-sMenu_item', _.bind(function (ev) {
        var $this = $(ev.currentTarget);
        if($this.hasClass('third')){
          //
          $this.siblings().removeClass('is-select').end().addClass('is-select');
        }else{
          //
    ...
    clearItemsState:function(){
      ...
      this.arrows.each(function(i,e){
        $(e).attr('src', $(e).attr('data-noopen')).css('transform', 'rotate(0deg)')
      });
      this.secondMenus.slideUp().find('.fj-sider-sMenu_item').removeClass('is-select');
    },
    setInitState: function (setedHash) {
      ...
      _.each(this.navData, function (item, i) {
        if (item.route == initHash) {
          this.firstItems.eq(i).addClass('is-opened').addClass('is-own');
          this.icons.eq(i).attr('src', this.icons.eq(i).attr('data-select'));
          this.arrows.eq(i).attr('src', this.arrows.eq(i).attr('data-open')).css('transform', 'rotate(0deg)');
        } else {
          _.each(item.children, function (item2, j) {
            if (item2.route == initHash) {
              this.firstItems.eq(i).addClass('is-opened');
              this.arrows.eq(i).css('transform', 'rotate(90deg)');
              this.secondMenus.eq(i).slideDown().find('.fj-sider-sMenu_item').eq(j).addClass('is-select');
            }
          }, this);
        }
      }, this);
    },
    unbindFirstItemsClickFn:function(){ //解绑一级菜单项的点击事件
      _.each(this.firstItems,function(item){
        var oItem = $(item);
    ...
    bindFirstItemsClickFn:function(){  //一级菜单项绑定点击事件
      this.firstItems.off('click').on('click', _.bind(this.firstItemsClickEvent,this));
      return this;
    },
    clearIconsClass:function(){ //一级导航的图标转为未选中
      this.icons.each(function(i,e){
        var $e = $(e);
        $e.attr('src', $e.attr('data-noSelect'));
      });
    },
    closeSecondMenus:function(){ //收起二级菜单
      this.secondMenus.stop(true,true).slideUp();
    },
    clearArrowsState:function(){ //取消箭头的选中样式
      this.arrows.each(function(i,e){
        var $e = $(e);
        $e.attr('src', $e.attr('data-noopen'));
      });
    },
    cancelArrowsRotate:function(){ //取消箭头的旋转
      this.arrows.css('transform', 'rotate(0deg)');
    },
    siderOpen2Close:function(){ //菜单展开->收起修改图标
      _.each(this.firstItems,function(item,i){
        var oItem = $(item);
    ...
    siderClose2Open:function(){ //菜单收起->展开修改图标
      this.icons.each(_.bind(function(i,e){
        var $e = $(e);
        if(!this.firstItems.eq(i).hasClass('is-own')){
          $e.attr('src', $e.attr('data-noSelect'));
        }
        if(this.firstItems.eq(i).hasClass('is-opened')&&!this.firstItems.eq(i).hasClass('is-own')){
          this.secondMenus.eq(i).stop(true,true).slideDown();
        }
      },this));
    }
  };
    ...
    data: function () {
      return {  
        ...
        operateNavDatas:{
          ...
          [fjPublic.userRoles.sj]:function(){ //市局
            return this.navData;
          },
          [fjPublic.userRoles.cg]:function(){ //超级管理员
...
<style>
  ...
  .fj-sider.sider-toggle .fj-sider-sMenu {
    position:absolute;left:calc(100% + 20px);top:50%;width:180px;padding:10px 10px 0px;background-color:rgba(0, 0, 0, .85);
    box-shadow:0px 0px 6px rgba(0,0,0,.65);transform:translateY(-50%);
  }
...
.fj-sider .fj-sider-menu_item > .title-box.is-own.is-opened:after {transform:rotate(0deg)} */
  ...
  .fj-sider.sider-toggle .fj-sider_slide {display:none;}
  /* 二级菜单 */
  .fj-sider-sMenu {
    position:relative;
    display: none;
  ...
  /* 三级菜单 */
  .fj-sider-sMenu_item > .fj-sider-sMenu.third {
    position:absolute;left:calc(100% + 20px);top:50%;width:180px;padding:10px 10px 0px;background-color:rgba(0, 0, 0, .85);
    box-shadow:0px 0px 6px rgba(0,0,0,.65);transform:translateY(-50%);
  }
  .fj-sider-sMenu_item:hover > .fj-sider-sMenu.third{
    display:block;
  }
  .fj-sider-sMenu_item > .fj-sider-sMenu.third > .fj-sider-sMenu_item.third {
    padding-left:0px;text-align:center;
  }
  .fj-sider-sMenu_item > .fj-sider-sMenu.third:after {
    content:'';position:absolute;left:-20px;top:0;bottom:0;width:20px;
  }
  .fj-sider-sMenu_item > .fj-sider-sMenu.third > .fj-sider-sMenu_item.third:after {display:none;}
